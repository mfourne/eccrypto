...
-- Re Timing-Attacks: We depend on (==) being resistant.
-- 
-----------------------------------------------------------------------------
...
pdouble (ECb _ alpha _ p _) p1@(ECPpF2 x1 y1 z1) =
  if x1==fromInteger 0 && y1==fromInteger 1 && z1==fromInteger 0
  then p1
  else let a = (x1 `F2.pow` (2::Int)) `F2.mod` p
           b = (a + (y1 * z1)) `F2.mod` p
           c = (x1 * z1) `F2.mod` p
           d = (c `F2.pow` (2::Int)) `F2.mod` p
           e = ((b `F2.pow` (2::Int)) + (b * c) + (if alpha==1 then d else 0)) `F2.mod` p
           x3 = (c * e) `F2.mod` p
           y3 = (((b + c) * e) + ((a `F2.pow` (2::Int)) * c)) `F2.mod` p
           z3 = (c * d) `F2.mod` p
       in ECPpF2 x3 y3 z3
...
padd curve@(ECb _ alpha _ p _) p1@(ECPpF2 x1 y1 z1) p2@(ECPpF2 x2 y2 z2)
        ...
        | x1==fromInteger 0 && y1==fromInteger 1 && z1==fromInteger 0 = p2
        | x2==fromInteger 0 && y2==fromInteger 1 && z2==fromInteger 0 = p1
        | p1==p2 = pdouble curve p1
        | otherwise = 
            let a = ((y1 * z2) + (z1 * y2)) `F2.mod` p
                b = ((x1 * z2)  +  (z1 * x2)) `F2.mod` p
                c = (b `F2.pow` (2::Int)) `F2.mod` p
                d = (z1 * z2) `F2.mod` p
                e = ((((a `F2.pow` (2::Int)) + (a * b) + (if alpha==1 then c else 0)) * d) + (b * c)) `F2.mod` p
                x3 = (b * e) `F2.mod` p
                y3 = (((c * ((a * x1) + (y1 * b))) * z2) + ((a + b) * e)) `F2.mod` p
                z3 = ((b `F2.pow` (3::Int)) * d) `F2.mod` p
            in ECPpF2 x3 y3 z3
...
ison curve@(ECb _ alpha beta p _) pt@(ECPpF2 _ _ _) = 
  let x = getxA curve pt
      y = getyA curve pt
  in ((y `F2.pow` (2::Int)) + (x * y)) `F2.mod` p == ((x `F2.pow` (3::Int)) + (if alpha==1 then (x `F2.pow` (2::Int)) else 0) + beta) `F2.mod` p
ison _ _ = undefined
{-# INLINABLE ison #-}
